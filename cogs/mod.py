...
from discord import Embed, Colour, Member, Object
...
class Mod(Cog):
    ...
    async def unban(self, ctx, members: Greedy[Object], *, reason=None):
        ...
        await ctx.send(
            embed=Embed(
                title="<:Mark:663230689860386846> Member(s) successfully unbanned",
    ...
    async def tempban(self, ctx, members:Greedy[Member], seconds, *, reason=None):
        if membrs := [m for m in members if m.top_role.position > ctx.author.top_role.position]:
            return await ctx.send(f"{membrs[0] if len(membrs) == 1 else ' and '.join([', '.join(map(str, membrs[:-1])), str(membrs[-1])])} can't be banned as their top role is higher than yours")
        if not reason: reason = f"Unspecified by {ctx.author}"
        if not seconds: return await ctx.send('You must say seconds to wait before unbanning after banning.')
        if not tryInt(seconds):
            if not members:
                return await ctx.send("There was a problem. If you pinged one or more members that have the same name as a certain role (most likely a bot), make sure you didn't accidentally ping the role instead of the member")
            return await ctx.send(f"Seconds must be full numbers and greater than 0, not \"{seconds}\"")
        if not ctx.author.guild_permissions.ban_members: return await ctx.send("This command requires you have `Ban Members` permission.")
        if not ctx.guild.me.guild_permissions.ban_members: return await ctx.send("This command requires I have `Ban Members` permission.")
        ...
        for member in members:
            if ctx.guild.me.top_role.position < member.top_role.position:
                wontBan.append(str(member))
            else:
                toBan.append(member)

        if toBan:
            for member in toBan:
                if member in (ctx.guild.me, ctx.author) and len(members) == 1:
                    return await ctx.message.add_reaction('🤔')
                try:
                    await ctx.guild.ban(member, reason=reason); banned.append(member.id)
                except:
                    pass
        ...
        if wontBan:
            embed.description = (embed.description or "") + (f"I can't ban the following members therefore skipped them. One or more of their top roles are over my top role. (This can be changed in the settings role section)\n```\n{', '.join(map(str, wontBan))}\n```\n" if len(wontBan)>1 else f"{wontBan[0]} hasn't been banned because one or more of his/her top roles are over my top role in the settings role section")
    ...
    async def timedban(self, ctx, members:Greedy[Member], seconds, *, reason=None):
        ...
        wontBan, toBan = [], []
        for member in members:
            if member in (ctx.guild.me, ctx.author) and len(members) == 1: return await ctx.message.add_reaction('🤔')
        ...
        embed = (Embed(colour=Colour.orange())
            .set_author(name='Timed ban', icon_url="https://i.imgur.com/7F7Q4x9.png")
    ...
    async def timedkick(self, ctx, members:Greedy[Member], seconds, *, reason=None):
        ...
        embed = (Embed(colour=Colour.orange())
            .set_author(name='Timed kick', icon_url="https://i.imgur.com/7F7Q4x9.png")
            .set_footer(text=f"Reason: {reason}"))
    ...
    async def kick(self, ctx, members:Greedy[Member], *, reason=None):
        ...
        if ignored:
            return await ctx.send(f"I couldn't kick {ignored[0] if len(ignored) == 1 else ' and '.join([', '.join(map(str, ignored[:-1])), str(ignored[-1])])} as my role is under one or more of theirs")

        await ctx.send(embed=Embed(title="<:Mark:663230689860386846> Member(s) successfully kicked", timestamp=datetime.now(), color = Colour.orange())
            .add_field(name="❔ Reason", value=f"```\n{reason}\n```" if not '`' in reason else reason, inline=True))
            # .set_thumbnail(url="https://i.imgur.com/7F7Q4x9.png"))
            ...
            for member in members:
                ...
                try:
                    ...
                    if find := mutedDB.find_one({"_id": f"{ctx.guild.id}"}):
                        ...
                        mutedDB.insert_one({"_id": f"{ctx.guild.id}", "muted": [f"{member.id}"]})
                except Forbidden:
                    print(format_exc())
        ...
        try:
            ...
            for member in members:
                try:
                    ...
                    return await ctx.send("Couldn't unmute some or all of the mentioned members, check my top role is above all theirs.")
                except:
                    print(format_exc())
                    ignored.append(member)
